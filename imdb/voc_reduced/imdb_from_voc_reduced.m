...
try
  ...
  VOCopts = get_voc_opts_reduced(root_dir,image_set, removed_classes);
  ...
  imdb.flip = flip;
  if flip
      image_at = @(i) sprintf('%s/%s.%s', imdb.image_dir, imdb.image_ids{i}, imdb.extension); %qui bisogna costrursi il p[ath verso le immagini
  ...
  imdb.eval_func = @imdb_eval_voc_reduced;
  imdb.roidb_func = @roidb_from_voc_reduced;
  imdb.image_at = @(i) ...
