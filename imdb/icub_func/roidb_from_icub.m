...
% ------------------------------------------------------------------------
function rec = attach_proposals(icub_rec, boxes, class_to_id, exclude_difficult_samples, flip)
% ------------------------------------------------------------------------
...
if ~isempty(boxes)
    boxes = boxes(:, [2 1 4 3]);
    if flip
        boxes(:, [1, 3]) = icub_rec.imgsize(1) + 1 - boxes(:, [3, 1]);
    end
end

%           gt: [2108x1 double]
%      overlap: [2108x20 single]
%      dataset: 'icub_trainval'
%        boxes: [2108x4 single]
%         feat: [2108x9216 single]
%        class: [2108x1 uint8]
if isfield(icub_rec, 'objects')
  if exclude_difficult_samples
      valid_objects = ~cat(1, icub_rec.objects(:).difficult);
  else
      valid_objects = 1:length(icub_rec.objects(:));
  end
  gt_boxes = cat(1, icub_rec.objects(valid_objects).bbox);
  if flip
    gt_boxes(:, [1, 3]) = icub_rec.imgsize(1) + 1 - gt_boxes(:, [3, 1]);
  end
  all_boxes = cat(1, gt_boxes, boxes);
  gt_classes = class_to_id.values({icub_rec.objects(valid_objects).class});
