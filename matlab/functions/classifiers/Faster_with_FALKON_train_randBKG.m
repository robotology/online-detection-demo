function [ rcnn_model ] = Faster_with_FALKON_train_randBKG(train_options, config, cnn_model, imdb, negatives_selection, rebalancing, fid, varargin )
...
%% Save models
% model_name = ['rls_model_norm' '_randBKG_' int2str(opts.negatives_selection.N)  '_sigma_' strrep(num2str(opts.train_classifier_options.sigma),'.','') ...
%               '_lambda_' strrep(num2str(opts.train_classifier_options.lambda),'.','')];
%           
%           
...
% end
% empty object and callback
cobj = [];
callback = @(alpha, cobj) [];
% ntr = size(X,1);                               % matrix of the Nystrom centers from the training set
% trp = randperm(ntr,opts.train_classifier_options.N_centres);
% opts.train_classifier_options.C = X(trp,:);
opts.train_classifier_options.C = X; % Avoid doing Nystrom
model.opts = opts.train_classifier_options;
model.alpha = falkon(X , opts.train_classifier_options.C , opts.train_classifier_options.kernel, y, opts.train_classifier_options.lambda, ...
                                                     opts.train_classifier_options.T, cobj, callback, opts.train_classifier_options.memToUse, ...
                                                     opts.train_classifier_options.useGPU);
                                                 
% model = gurls_train(X, y, opts.train_classifier_options);
end
