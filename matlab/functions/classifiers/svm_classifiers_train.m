function [model_classifier, model_classifier_kfold] = ...
    svm_classifiers_train(config, imdb, cnn_model, varargin)
...
for i = imdb.class_ids
  ...
  X_pos{i} = SVM_scale_features(X_pos{i}, opts.feat_norm_mean);
...
% ------------------------------------------------------------------------
if opts.k_folds > 0
  model_classifier_kfold = rcnn_model;
  [W, B, folds] = update_model_k_fold(rcnn_model, caches, imdb);
  model_classifier_kfold.folds = folds;
  for f = 1:length(folds)
    model_classifier_kfold.detectors(f).W = W{f};
    model_classifier_kfold.detectors(f).B = B{f};
  end
  save([conf.cache_dir 'rcnn_k_fold_model'], 'rcnn_k_fold_model');
else
  model_classifier_kfold = [];
end
% ------------------------------------------------------------------------
