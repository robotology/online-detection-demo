function [ rcnn_model ] = Faster_with_FALKON_train_randBKG_no_norm(train_options, config, cnn_model, imdb, negatives_selection, rebalancing, fid, varargin )
...
save_file_negative = sprintf('./feat_cache/%s/%s/gt_neg_%d%s_cache_no_norm.mat', rcnn_model.cache_name, imdb.name,opts.negatives_selection.N, opts.negatives_selection.policy);
