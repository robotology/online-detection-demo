function [ rcnn_model ] = Faster_with_FALKON_train_randBKG_try_try(train_options, config, cnn_model, imdb, negatives_selection, rebalancing, fid, varargin )
...
save_file_negative = sprintf('./feat_cache/%s/%s/gt_neg_%d%s_cache_half.mat', rcnn_model.cache_name, imdb.name,opts.negatives_selection.N, opts.negatives_selection.policy);
try
...
%   X_neg = X_neg([1:2:size(X_neg,1)],:);
  print('%d', size(X_neg,1));
end
...
%   X_neg{i} = X_neg{i}([1:2:size(X_neg{i},1)],:);
  X_neg{i} = zscores_standardization(X_neg{i}, opts.standard_deviation, opts.mean_feat,  opts.mean_norm);
%   X_neg{i} = rcnn_scale_features_try_try(X_neg{i}, opts.mean_norm, opts.train_classifier_options.target_norm);
%   fprintf('%14s has now %6d negative instances\n', imdb.classes{i}, size(X_neg{i},1));
%   fprintf(fid, '%14s has now %6d negative instances\n', imdb.classes{i}, size(X_neg{i},1));
