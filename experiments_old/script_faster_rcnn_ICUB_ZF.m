...
opts.do_val                 = true; %ELISA: todo
% model
model                       = Model.ZF_for_Faster_RCNN_VOC0712; %ELISA: to change
% cache base
cache_base_proposal         = 'faster_rcnn_ICUB_ZF'; %ELISA: to change
cache_base_fast_rcnn        = '';
% train/test data
dataset                     = [];
use_flipped                 = true;
dataset                     = Dataset.icub_trainval(dataset, 'train', use_flipped); %ELISA: changed
dataset                     = Dataset.icub_test(dataset, 'test', false); %ELISA: to change
