...
%% -------------------- INIT_MODEL --------------------
model_dir                   = fullfile(pwd, 'output', 'faster_rcnn_final', 'faster_rcnn_ICUB_ZF'); %% VGG-16
...
while ischar(tline)
%     disp(tline)
    im_names{count} = fullfile('datasets/iCubWorld-Transformations_devkit/Images' , strcat(tline, '.jpg'));
    tline = fgets(fid);
    count = count + 1;
end
...
for j = 1:length(im_names)
    ...
    imagename = strcat('prova/',int2str(j));
    print(imagename, '-djpeg');
    pause(0.1);
    close(f);
end
