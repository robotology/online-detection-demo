function script_faster_rcnn_VOC2007_ZF_TASK2()
...
opts.do_val                 = false; 
% model
model                       = Model.ZF_for_Faster_RCNN_VOC2007_TASK2;
% cache base
...
output_dir                        = 'output_VOC2007_TASK2';
...
fprintf('\n***************\nstage two fast rcnn\n***************\n');
% train
%Extracting regions from train-set task2 using features from task 1
dataset.TASK2.roidb_train       	= cellfun(@(x, y) Faster_RCNN_Train.do_proposal_test(conf_proposal, model.stage2_rpn,output_dir, x, y), dataset.TASK2.imdb_train, dataset.TASK2.roidb_train, 'UniformOutput', false);

% model.stage2_fast_rcnn.init_net_file = model.stage1_fast_rcnn.output_model_file;
model.stage2_fast_rcnn      = Faster_RCNN_Train.do_fast_rcnn_train(conf_fast_rcnn, dataset.TASK2, model.stage2_fast_rcnn, opts.do_val, output_dir);

%% final test
fprintf('\n***************\nfinal test\n***************\n');
model.stage2_rpn.nms        = model.final_test.nms;
fprintf('saving workspace...');
save('workspaces/ZF_voc2007_finetune_TASK2');

%fprintf('saving models...');
Faster_RCNN_Train.gather_rpn_fast_rcnn_models(conf_proposal, conf_fast_rcnn, model, dataset.TASK2);
...
dataset.TASK2.roidb_test       	= cellfun(@(x, y) Faster_RCNN_Train.do_proposal_test(conf_proposal, model.stage2_rpn,output_dir, x, y), dataset.TASK2.imdb_test, dataset.TASK2.roidb_test, 'UniformOutput', false);
opts.final_mAP                  = cellfun(@(x, y) Faster_RCNN_Train.do_fast_rcnn_test(conf_fast_rcnn, model.stage2_fast_rcnn,output_dir, x, y), dataset.TASK2.imdb_test, dataset.TASK2.roidb_test, 'UniformOutput', false);
save('workspaces/ZF_voc2007_finetune_TASK2');
