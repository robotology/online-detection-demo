function script_FALKON_voc2007_ZF_CV_no_norm(bkg_numb, sigmas, lambdas, Nyst_centres, iterations, gpu_id)
clear mex;
gpuDevice(gpu_id);

FALKON_voc_no_norm_init_variables;
...
results.matrix_cls = zeros(length(Nyst_centres), length(sigmas), length(lambdas), length(iterations));
results.matrix_reg = zeros(length(Nyst_centres), length(sigmas), length(lambdas), length(iterations));
    ...
    for s=1:length(sigmas)
        for l =1:length(lambdas)
            for t=1:length(iterations)
                ...
                results.matrix_cls(m,s,l,t) = mean(aps_cls);
                % test regressors
                ...
                results.matrix_reg(m,s,l,t) = mean(aps_reg);
                save([results_dir '/results_no_norm.mat'], 'results');
            end
        end
    end
end
