function script_FALKON_on_iCWT_TASK2_miniBootstrap_FC6features(bkg_numbs, sigmas, lambdas, nyst_centres, iterations, gpu_id)

gpuDevice(gpu_id);

FALKON_iCWT_TASK2_miniBtstrp_init_variables_FC6;
...
best_model = {};
file_name = [results_dir '/test_Bootstrap.txt'];
...
for m=1:length(nyst_centres)
    ...
    for k=1:length(sigmas)
        file_name = [results_dir '/precisions_M' int2str(nyst_centres(m)) '_sigma' int2str(sigmas(k)) '.txt'];
