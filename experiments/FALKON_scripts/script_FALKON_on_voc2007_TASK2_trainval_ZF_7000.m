function script_FALKON_on_voc2007_TASK2_trainval_ZF_7000(bkg_numbs, sigmas, lambdas, nyst_centres, iterations, gpu_id)

gpuDevice(gpu_id);

FALKON_voc_trainval_7000_TASK2_init_variables;

% region proposals generation
dataset.TASK2.roidb_train  = cellfun(@(x, y) Faster_RCNN_Train.do_proposal_test(conf_proposal,model.stage2_rpn, output_dir, x, y, 'suffix', TASK2_proposals_suffix), ...
...
% region proposals generation
dataset.TASK2.roidb_test = cellfun(@(x, y) Faster_RCNN_Train.do_proposal_test(conf_proposal, model.stage2_rpn , output_dir, x, y, 'suffix', TASK2_proposals_suffix), ...
...
if(train_classifier_options.cross_validation.required)
    ...
    dataset.TASK2                     = Dataset.voc2007_train(dataset.TASK2, 'val', use_flipped, removed_classes_T2, imdb_cache_name);
    dataset.TASK2.roidb_val = cellfun(@(x, y) Faster_RCNN_Train.do_proposal_test(conf_proposal, model.stage2_rpn , output_dir, x, y, 'suffix', TASK2_proposals_suffix), ...
    ...
    for k=1:length(sigmas)
        ...
        for l=1:length(lambdas)
            ...
            res_cls = cellfun(@(x) Incremental_Faster_RCNN_Train.do_classifiers_test(train_classifier_options.cache_dir, conf_fast_rcnn, '',  cls_mode, ...
                                                                              model.classifiers.falkon{1}, x, fid), dataset.TASK2.imdb_val, 'UniformOutput', false);
            ...
            res_reg = cellfun(@(x) Incremental_Faster_RCNN_Train.do_regressor_test(conf_fast_rcnn, model.bbox_regressors, model.feature_extraction, x, fid), ...
                                                                              dataset.TASK2.imdb_val, 'UniformOutput', false);
...
save([results_dir '/best_model_' int2str(bkg_numbs)  'bkg.mat'], 'model');  
