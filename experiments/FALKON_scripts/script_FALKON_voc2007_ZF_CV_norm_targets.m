function script_FALKON_voc2007_ZF_CV_norm_targets(bkg_numb, sigmas, lambda, Nyst_centres, iterations, target_norms, gpu_id)
clear mex;
gpuDevice(gpu_id);

CV_norm_FALKON_voc_init_variables;
...
results.matrix_cls = zeros(length(Nyst_centres),length(iterations),length(sigmas),length(target_norms));
results.matrix_reg = zeros(length(Nyst_centres),length(iterations),length(sigmas),length(target_norms));
results.Nyst_centres=Nyst_centres;
for m=1:length(Nyst_centres)
    for t=1:length(iterations)
        for s=1:length(sigmas)
            for n=1:length(target_norms)
                train_classifier_options.M=Nyst_centres(m);
                train_classifier_options.T=iterations(t);
                train_classifier_options.sigma=sigmas(s);
                train_classifier_options.target_norm=target_norms(n);
                ...
                fprintf(fid, 'target_norm: %d\n',train_classifier_options.target_norm);
                ...
                save([results_dir '/results.mat'], 'results');
            end
        end
    end
end
