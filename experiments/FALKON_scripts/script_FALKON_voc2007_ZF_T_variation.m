function script_FALKON_voc2007_ZF_T_variation(bkg_numb, sigma, lambda, iterations, gpu_id)
clear mex;
gpuDevice(gpu_id);

TVar_FALKON_voc_init_variables;
...
results = struct;
results.matrix_cls = zeros(length(iterations));
results.matrix_reg = zeros(length(iterations));
results.T=iterations;
for t=1:length(iterations)
    train_classifier_options.T=iterations(t);
    fprintf(fid, 'Num of iters: %d', train_classifier_options.T);
    ...
    results.matrix_cls(t) = mean(aps_cls);
    % test regressors
    ...
    results.matrix_reg(t) = mean(aps_reg);
