function script_faster_rcnn_VOC2012_ZF()
...
opts.do_val                 = false; 
% model
model                       = Model.ZF_for_Faster_RCNN_incremental_VOC2012;
% cache base
cache_base_proposal         = 'faster_rcnn_VOC2012_zf_layers';
cache_base_fast_rcnn        = '';
% train/test data
dataset                     = [];
use_flipped                 = false;
dataset                     = Dataset.voc2012_trainval(dataset, 'train', use_flipped);
dataset                     = Dataset.voc2012_val(dataset, 'test', false);
